...
{
	std::list<Rubbish>::iterator r_it;
	...
	{
		{
			...
			if (untreated_list.size() > 0)
			{
				...
				{
					if ((*it).GetState() == false)      //找到第一个空闲状态的工人
					{
						mtx.lock();
						Rubbish temp(0);
						for (r_it = untreated_list.begin(); r_it != untreated_list.end();r_it++)	//从待处理垃圾链表中取出一个属于他的区域的垃圾
						{
							if ((*r_it).GetPosition().GetX() >= (*it).GetMinX() && (*r_it).GetPosition().GetY() >= (*it).GetMinY() &&
								(*r_it).GetPosition().GetX() < (*it).GetMaxX() && (*r_it).GetPosition().GetY() < (*it).GetMaxY())	//如果有，取出该垃圾
							{
								temp = (*r_it);
								untreated_list.erase(r_it);
								flag = true;
								break;
							}
						}
						if (flag == true)	//将该垃圾分配给相应环卫工
						{
							(*it).SetState(true);
							(*it).Allocate(temp);
